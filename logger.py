...
class Logger(object):
    ...
    def print_statistics(self, run=None):
        if run is not None:
            result = np.array(100) * np.array(self.results[run]) 
            argmax = result[:, 1].argmax().item()
            print(f'Run {run + 1}:')
            print(f'Highest Train: {result[:, 0].max()}')
            print(f'Highest Valid: {result[:, 1].max()}')
            print(f'  Final Train: {result[argmax, 0]}')
            print(f'   Final Test: {result[argmax, 2]}')
        else:
            result = np.array(100) * np.array(self.results) 
            best_results = []
            for r in result:
                train1 = r[:, 0].max().item()
                valid = r[:, 1].max().item()
...
class SimpleLogger(object):
    ...
    def get_best(self, top_k=1):
        ...
        for args in self.used_args:
            ...
            results = paddle.tensor(results)*100
